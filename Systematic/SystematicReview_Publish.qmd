---
title: "SystematicReview_Final" # MEL consider linking title to manuscript?
                                # e.g., "Analysis code associated with Hoffman et al..."
                                # just to make it super obvious for reviewers
author: [fill in]
format: html
editor: visual
---
...
Cao 2016 & Jia 2022: Poyang \#### MEL can you explain why Poyang was excluded?
...
Geraldes 2007 - this paper presented two reservoirs. For the Serra Serrada Reservoir, the data was duplicate Geraldes 2005. Azivo Reservoir is still included from Geraldes 2007. \#### MEL Katie check my edits are true here?
...
Sharip: paper got duplicated, removed the duplicate version. Also, Bera lake not more than 4.5m deep, so does not meet our depth criterion.
...
data <- data_excl |> 
  ...
  mutate(elev_decrease_max = ifelse(grepl("2001", elev_decrease_max), "-6", elev_decrease_max), # to use the 2003 value MEL are you meaning to search for 2001 in the elev_decrease_max column? seems that you are trying to change from a value reported for a 2001 paper to a value reported for a 2003 paper; if that's what you are trying to do, this is a typo
...
trophic
# MEL I've revised all of these ggsaves to be relative filepaths; this was creating a whole new directory on my computer that was not where the RProject was
...
Physical variables that we considered as potential drivers of phytoplankton community change: water temperature, water level, Secchi disk depth/transparency, light(PAR, photic zone), turbidity, total suspended solids, total dissolved solids, other

Physical variables that we considered as potential drivers of phytoplankton community change: pH, DO, DOC, DIC, DOM, POM, TP, SRP, ORP (orthophosphate), TN, TKN, NO3, NO2, NH4, DIN, metals, conductivity, salinity, other
...
\[MEL Add short description of chunk here\]
...
chem <- chemicalvars_phytos |> 
  ...
  ggtitle("Chem variable drivers, phytoplankton only" ) + 
...
phytos <- phys/chem
phytos
# MEL what are id and notid again?
...
cyanos
# MEL same question, can you remind me what id and notid are?
...
\[MEL can you add a brief description of what it means to prioritize water level decreases, prioritize water level increases, and use both? Just something really short, maybe 2-3 sentences; in fact you could just paraphrase from manuscript\]
...
```{r}
# MEL the data wrangling from lines 1002 - 1121 is intense! which I know is sort of on me since you were adapting code I wrote, I think :-) ...I got a little lost trying to follow some of the similar column names - can you provide a little annotation/comments to help guide a reviewer through this section?
dat <- boxplotdata2 %>% 
...
#### MEL note to Katie to remove these colors once finalized
...
phyto_chi <-  ggplot(dat_phyto_new, aes(x = increase_decrease_mosaic, y = count, fill = increase_phyto))  +
  ...
  theme(#axis.text.x = element_text(hjust = 0.5, size = 20),
        ...
        axis.text.y = element_text(hjust = 1, size = 20),
        axis.title.y=element_text(angle=90, hjust = 1, size = 20),
...
cyano_chi <- ggplot(dat_cyano_new, aes(x = increase_decrease_mosaic, y = count, fill = increase_cyano)) + 
    geom_bar(position="stack", stat="identity")+ 
    xlab("") + ylab("") +
   geom_text(aes(label = count),
            position = position_stack(vjust = 0.5), 
...
```
#### MEL from here to the end of the document, it seems that this is all archived code - if so, I would recommend to move this into a separate file and name it something like archived analyses or exploratory analyses so that reviewers don't spend time here, since it's not actually being used for the paper. If you do end up using some of this code for the paper in the course of revisions, then you can move it back into this file.
Old mapping figures
...
#             color = "white", 
#             size = 3)
ggsave(paste0(here(),"/Systematic/Plots/varswlchange.jpg"), plot = deltawl, height = 5, width = 6.5, units = "in")
ggsave(paste0(here(),"/Systematic/Plots/vars.jpg"), plot = varsplot, height = 5, width = 6.5, units = "in")
