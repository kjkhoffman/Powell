...
```{r exclude papers}
data_excl <- init_data |> 
  filter(!name_yr %in% c("Beaver 2013","Jia 2022","Cao 2016","Wang 2022","Kisand 2004", "Hart 2004")) |> 
...
test <- data |> 
  ...
  mutate(water_level = ifelse(grepl("water level", phys_vars), 1, 0), 
         ...
         tss = ifelse(grepl("total suspended solids", phys_vars), 1, 0), 
...
physvars <- test |> select(c(water_level, water_temp, transparency, turb, light, tss, other_phys))
summary(vars) 
...
drivers <- data |> 
  ...
  mutate(water_level = ifelse(grepl("water level", phys_drive), 1, 0), 
         ...
         tss = ifelse(grepl("total suspended solids", phys_drive), 1, 0), 
         other_phys = ifelse(grepl("Other", phys_drive), 1, 0))
...
chemvars <- drivers |> select(c(other_chem, tp, tn, nh4, ph, do, srp, no3, cond, metals, orp, no2, doc, din, ph, salinity))
physvars <- drivers |> select(c(water_level, water_temp, transparency, turb, light, tss, other_phys))
# summary(vars) 
...
rm(physvars)
physvars2 <- setNames(cbind(rownames(freqphys), freqphys, row.names = NULL), 
         c("names", "sum"))

physvars2 |> ggplot(aes(x = names, y = sum)) + geom_bar(stat = "identity") + 
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=1)) + 
