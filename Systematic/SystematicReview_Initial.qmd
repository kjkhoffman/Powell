...
```{r exclude papers}
data_excl <- init_data |> 
  filter(!name_yr %in% c("Beaver 2013","Jia 2022","Cao 2016","Wang 2022","Kisand 2004", "Hart 2004", "Nõges 2010", "Tuvikene 2011", "Noges 1999", "Abirhire 2019", "Braga 2015", "Sakharova 2018", "Lacerda 2018")) |>
  filter(res_name != la"Dourado" | name_yr != "deMednonca 2018") |> 
  filter(!res_name == "Serra Serrada Reservoir") |>  #this is one of the reservoirs in Geraldes 2007
...
excluded_papers <- init_data |> 
  filter(name_yr %in% c("Beaver 2013","Jia 2022","Cao 2016","Wang 2022","Kisand 2004", "Hart 2004", "Nõges 2010", "Tuvikene 2011", "Noges 1999", "Abirhire 2019", "Braga 2015", "Sakharova 2018", "Lacerda 2018")|res_name == "Serra Serrada Reservoir"| title == "Steady-state assemblages in a Mediterranean hypertrophic reservoir. The role of Microcystis ecomorphological variability in maintaining an apparent equilibrium"|notes %in% c("Ignore this version, everything has been added to multiple reservoirs sheet! KKH")| res_name == "Dourado" & name_yr != "deMednonca 2018") 
...
This set of chunks is for histograms of the categorical information that
we pulled. Number of waterbodies Country fullpond_maxdepth
fullpond_maxvol trophic_status decrease_phyto decrease_cyano
increase_phyto increase_cyano
...
latlong <- data %>%
  select("lat_dd", "long_dd", "res_name", "decrease_phyto_response", "increase_phyto_response", "decrease_cyano_response", "increase_cyano_response") |> 
...
sitemap <- ggplot() +
  ...
  geom_sf(data = litdata_map_sf,
          ...
          size = 2) +
  #scale_fill_manual(values = c("white", "#88CCEE"), name = "Countries") +
  scale_color_manual(values = c("#fba049", "#3532fb", "#CC6677"),
                     name = "Variable") +
...
Could use this code to get both halves of water level increases and
decreases.
...
Chi squares based on Mary's code. I think this is correct but not 100%,
have someone take a look before pubs.
...
phyto_chi <-  ggplot(dat_phyto_new, aes(x = increase_decrease_mosaic, y = count, fill = increase_phyto))  +
   ...
   geom_text(aes(label = count),
            ...
            color = "black", 
            size = 6) + 
  scale_fill_manual(values = c("#d2d2fe", "#fba049"))+ theme_bw()
  ...
  ggplot(dat_phyto_new, aes(x = increase_decrease_mosaic, y = count, fill = increase_phyto)) + 
   ...
   geom_text(aes(label = count),
            ...
            color = "black", 
            size = 10) + 
  scale_fill_manual(values = c("#d2d2fe", "#fba049"))
...
phyto_only_WLchangevars <- WLchangevars |> 
  select(c(1:3, increase_phyto_response, increase_cyano_response, decrease_phyto_response, decrease_cyano_response, tp, 90:121)) |> 
  filter(increase_phyto_response != "" & increase_phyto_response != "Not measured" | decrease_phyto_response != "" & decrease_phyto_response != "Not measured")
...
phyto_only_vars <- spreadvars |> 
  select(c(1:3, increase_phyto_response, increase_cyano_response, decrease_phyto_response, decrease_cyano_response, tp, 90:126)) |> 
  filter(increase_phyto_response != "" & increase_phyto_response != "Not measured" | decrease_phyto_response != "" & decrease_phyto_response != "Not measured")
...
#many regerts
chemvars_drive_phytos <- phyto_only_drivers |> select(c(other_chem, tp, tn, nh4, ph, do, srp, no3, cond, metals, orp, no2, doc, din, ph, salinity))

# chemvarsWL_phytos <- phyto_only_WLchangevars |> select(c(other_chem, tp, tn, nh4, ph, do, srp, no3, cond, metals, orp, no2, doc, din, ph, salinity))

chemvars_phytos <- phyto_only_vars |> select(c(other_chem, tp, tn, nh4, ph, do, srp, no3, cond, metals, orp, no2, doc, din, ph, salinity))

physvars_drive_phytos <- phyto_only_drivers |> select(c(water_level, water_temp, transparency, turb, light, tss_tds, other_phys, strat, restime, flush_discharge, inflow, precip, air_temp))
...
physvars_phytos <- phyto_only_vars |> select(c(water_temp, transparency, turb, light, tss_tds, other_phys, strat, restime, flush_discharge, inflow, precip, air_temp, water_level))
# summary(vars) 
...
```{r cyanos vars only}
chemvars_drive_cyanos <- cyano_only_drivers |> select(c(other_chem, tp, tn, nh4, ph, do, srp, no3, cond, metals, orp, no2, doc, din, ph, salinity))

# chemvarsWL_phytos <- phyto_only_WLchangevars |> select(c(other_chem, tp, tn, nh4, ph, do, srp, no3, cond, metals, orp, no2, doc, din, ph, salinity))

chemvars_cyanos <- cyano_only_vars |> select(c(other_chem, tp, tn, nh4, ph, do, srp, no3, cond, metals, orp, no2, doc, din, ph, salinity))

physvars_drive_cyanos <- cyano_only_drivers |> select(c(water_level, water_temp, transparency, turb, light, tss_tds, other_phys, strat, restime, flush_discharge, inflow, precip, air_temp))

# physvarsWL_phytos <- phyto_only_WLchangevars |> select(c(water_temp, transparency, turb, light, tss_tds, other_phys, strat, restime, flush_discharge, inflow, precip, air_temp))

physvars_cyanos <- cyano_only_vars |> select(c(water_temp, transparency, turb, light, tss_tds, other_phys, strat, restime, flush_discharge, inflow, precip, air_temp, water_level))
...
# physvars2_drive_cyanos |> ggplot(aes(x = names, y = sum)) + geom_bar(stat = "identity") + 
#   theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=1)) + ggtitle("Phys variable Drivers")
...
physvars_cyanos |> 
  ...
  geom_bar(position = "fill", stat = "identity") +
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=1)) + 
  ggtitle("Phys variables, cyano only") + 
  geom_text(aes(label = count),
            position = position_fill(vjust = 0.5), 
            color = "black", 
            size = 3)
...
chemvars_drive_phyto_down <- phyto_only_drivers_down |> select(c(other_chem, tp, tn, nh4, ph, do, srp, no3, cond, metals, orp, no2, doc, din, ph, salinity))

# chemvarsWL_phytos <- phyto_only_WLchangevars |> select(c(other_chem, tp, tn, nh4, ph, do, srp, no3, cond, metals, orp, no2, doc, din, ph, salinity))

chemvars_phyto_down <- phyto_only_down |> select(c(other_chem, tp, tn, nh4, ph, do, srp, no3, cond, metals, orp, no2, doc, din, ph, salinity))

physvars_drive_phytos <- phyto_only_drivers_down |> select(c(water_level, water_temp, transparency, turb, light, tss_tds, other_phys, strat, restime, flush_discharge, inflow, precip, air_temp))

# physvarsWL_phytos <- phyto_only_WLchangevars |> select(c(water_temp, transparency, turb, light, tss_tds, other_phys, strat, restime, flush_discharge, inflow, precip, air_temp))

physvars_phytos_down <- phyto_only_down |> select(c(water_temp, transparency, turb, light, tss_tds, other_phys, strat, restime, flush_discharge, inflow, precip, air_temp, water_level))

# physvars_cyanos <- phyto_only_down |> select(c(water_temp, transparency, turb, light, tss_tds, other_phys, strat, restime, flush_discharge, inflow, precip, air_temp, water_level))
# summary(vars) 

freqchem_drive_cyanos <- as.data.frame(colSums(chemvars_drive_phyto_down))
chemvars_drive_cyanos <- setNames(cbind(rownames(freqchem_drive_cyanos), freqchem_drive_cyanos, row.names = NULL), 
         c("names", "sum"))
# freqchemWL_cyano <- as.data.frame(colSums(chemvarsWL_cyanos))
...
# chemvarsWL_cyanos <- setNames(cbind(rownames(freqchemWL), freqchemWL_cyano, row.names = NULL), 
#          c("names", "sum"))
chemvars_cyanos <- setNames(cbind(rownames(freqchem_phyto_down), 
