...
phyto_data <- read_csv("~/GitHubRepos/CareyLabVT/Powell/Data/Lake Powell Data/PhytoplanktonData_LakePowell_1992-2022.csv", locale=locale(encoding="latin1"))
...
cyanos <- phyto_data |> 
  ...
  filter(StationID%in%forebay) |> 
  mutate(day = lubridate::mdy_hm(CollectionDateTime)) |> 
  mutate(day = date(day))

cyanos2 <- cyanos |> 
  pivot_wider(names_from = GenusSpecies, values_from = Abundance, values_fill = 0)
  group_by(year(day)) |>
  ggplot(aes(day, Abundance)) +
  geom_col(aes(fill = GenusSpecies)) + 
  facet_wrap(~year(day), scales = "free_x")

ggplot(cyanos, aes(fill=GenusSpecies, y=Abundance, x=year(day))) + 
    geom_bar(position="stack", stat="identity", show.legend = F) + theme_bw()

unique(cyanos$GenusSpecies)

mutate()
c("Aphanizomenon", "Chroococcus", "Microcystis", "Aphanocapsa", "Pseudanabaena", "Gomphosphaeria", "Snowella", "Dolichospermum", "Planktolyngbya", "Limnothrix", "Cylindrospermopsis", "Spirulina", 
  "Woronichinia", "Oscillatoria", "Aphanothece", "Planktothrix", "Lyngbya", "Komvorophon", 
  "Heteroleibleinia", "Anabaena", "Merismopedia", "Aphanothece", "Leptolyngbya", 
  "Gloeothece")         

cyanos |> 
  filter(year(day) == "1998"|year(day) == "2005") %>% 
  ggplot(aes(day, Abundance)) +
    geom_col(aes(fill = GenusSpecies)) + 
  facet_wrap(~year(day), scales = "free_x") +
# scale_fill_manual(values = c(phyto_colors), name="Division",
