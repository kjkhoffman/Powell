...
```{r log reg for poster, new colors}
cyanodecreasedata <- boxplotdata2 %>% 
  select(prop_delta_wl, combined_cyano_response, combined_cyano_response_binary, study_id) %>% 
  filter(!is.na(prop_delta_wl)) %>%
  filter(!is.na(combined_cyano_response_binary)) %>% 
...
ggsave("~/GitHubRepos/CareyLabVT/Powell/Plots/LitReview/GCDAMP_Poster/combinedlogregression.png", plot = test, height = 8.2, width = 14.5, units = "in")
```
...
mgmtconcerns <- concerns %>% 
  ...
  theme(text = element_text(size = 28),
        axis.text.x = element_text(angle = 0, hjust = .5, size = 20))

mgmtconcerns
ggsave("~/GitHubRepos/CareyLabVT/Powell/Plots/LitReview/GCDAMP_Poster/managementconcerns.png", plot = mgmtconcerns, height = 6.2, width = 6.2, units = "in")
